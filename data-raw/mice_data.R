...
mu <- rnorm(n = nrow(mice.A), mean = 10, sd = 2)
mice_mrna <- t(MASS::mvrnorm(n = 1200, mu = mu, Sigma = mice_ped))
mice_mrna <- mice_mrna[match(mrna_nms, rownames(mice_mrna)), ]
mice_pheno <- mice.pheno[, c("SUBJECT.NAME", "Obesity.BMI")]
